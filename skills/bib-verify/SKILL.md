---
name: bib-verify
description: Verify BibTeX citations against academic databases (Crossref, arXiv, OpenAlex, PubMed) and detect hallucinated, fabricated, or AI-generated references. Use whenever the user has a `.bib`, `.bibtex`, or `.bbl` file, a LaTeX paper with `\cite{}`, or any list of academic references and (a) explicitly asks to "check citations", "verify references", "find broken/fake/hallucinated citations", "check bibliography", "validate refs", "are these papers real", "proofread bib", "find broken DOIs", "did Claude hallucinate any refs", "audit my citations", or "make sure my references exist"; or (b) just received a reviewer comment complaining about fake/AI/hallucinated references; or (c) is preparing a paper submission and wants a final reference sanity check; or (d) just had Claude write or edit a `.bib` entry (run proactively to catch hallucinations before the user notices). Also use when the user pastes a single citation and asks "is this real?" or "verify this reference". Do NOT use this skill to generate new citations from scratch (no literature search), reformat BibTeX syntax (no bibtex-tidy replacement), or convert between citation styles.
---
